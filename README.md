...
- Toolbar → **Dashboard** opens the CFA readiness dashboard
  ...
  as coverage gaps). Each per-topic row also has a **Study** button
  (desktop-only) that opens a focused review of just that topic's cards.
- **Study a CFA topic** (desktop-only): review one topic instead of the
  whole deck. From the dashboard's per-topic **Study** button, or a deck's
  gear menu → **Study a CFA topic...**, the app (re)builds an Anki
  **filtered deck** from the topic's `cfa::topic::*` tag (plus any tags you
  mapped onto it) and drops you straight into review. It reuses Anki's own
  filtered-deck engine — reviews reschedule normally, exactly as if you
  studied those cards in place — so there is no scheduler change.
...
excluded from readiness). The card face shows the prompt, choices and the
answer rationale only; the note's **Source** field carries the canonical
`cfa::topic::*` tag (machine-readable provenance, keyed to the dashboard's
10 topics) rather than an on-card corpus citation — the deeper grounding
provenance (declared/retrieved passage per item) still lives in the pipeline
artifacts (`items/generated.jsonl`, `eval/validation_report.json`). The
ladder is inert until you enable **Fade Ladder** in deck options and set an
exam date on the dashboard. To regenerate the content and the deck:
...
- `python3 -m unittest discover -s tools/speedrun/tests` (or
  ...
  tools below: bench, crash/sync tests, memory calibration, card check).

### Measurement & proof commands (one command each)

Every claim in the results docs traces to a re-runnable command; reports
land in `desktop/tools/speedrun/eval/`. From `desktop/`:

| command | what it proves | report |
| --- | --- | --- |
| `just bench` | challenge 7h / §10: builds a 50,000-card collection and prints p50/p95/worst for button-ack, next-card, dashboard first load/refresh, session sync, cold start, peak memory — each against its §10 target | `eval/bench_report.md` |
| `just crash-test` | challenge 7g: SIGKILLs the engine mid-review 20×, proves zero corrupted collections; then network-off — AI abstains cleanly while Memory computes and Readiness abstains by design | `eval/crash_test_report.md` |
| `just sync-test` | challenge 7b: 10 offline reviews per client → all 20 land, none lost/double-counted; same-card conflict resolves by newest-mod-wins with both reviews kept | `eval/sync_test_report.md` |
| `just memory-calibration` | §9 Step 1: FSRS predicted vs observed recall on held-out (post-cutoff) reviews — Brier, log-loss, ECE, reliability chart; `--collection <path>` runs it on any real `.anki2` | `eval/memory_calibration_report.md` + `memory_calibration_chart.svg` |
| `just card-check` | challenge 7f: 50-pair hand-authored gold set (+15 seeded defects) validates the checker; 50 cards generated from ONE source scored correct-useful / wrong / bad-teaching against a cutoff frozen before scoring; failing cards are blocked (non-zero exit + sidecar) | `eval/card_check_report.md` |
| `just injection-eval` | §10 "source file with hidden text" / AI-safety: 6 hidden-text prompt-injection payloads run through every model-facing surface (card generator, onboarding, tag-suggester, assistant), driven by an adversarial backend that obeys them; proves the app never trusts model output | `eval/injection_eval_report.md` |
| `just probe-harness --collection <path>` | challenge 7d/7e: probe-bank validation + leakage scan + the real memory→performance bridge gap and readiness calibration from a collection (abstains below 10 delayed calibration outcomes) | `eval/probe_harness_report.md` |
| `just ablation` / `just ablation --collection <path>` | §8: the nine-arm simulated ablation, plus the real-collection **observational companion** (clearly labelled NOT an ablation; n=1 disclosed) | `eval/ablation_report.md` / `eval/ablation_real_report.md` |
| `bash tools/speedrun/android_crash_test.sh` | challenge 7g on Android: the same 20-kill loop against a connected device/emulator (fails fast when none is attached) | printed + pulled-DB integrity check |

The AI-vs-baseline retrieval side-by-side re-runs inside the pipeline
(`python3 aig/run_pipeline.py --backend mock` from `tools/speedrun/`) and
is adjudicated in `eval/retrieval_eval.md` — in the default stdlib
environment the tuned-BM25 keyword arm currently **beats** the stdlib
vector arm and the RRF fusion (P@1 0.500 vs 0.182 / 0.364), so the shipped
grounding stays BM25 and the [R21] "fusion beats both" claim is only
quoted from the archived full-stack (torch) run, clearly caveated.
...
| `desktop/tools/speedrun/{bench,crash_test,sync_test,memory_calibration,card_check}.py`, `android_crash_test.sh`, `gold/` | **Final (measurement & proof)**: the 50k-card §10 benchmark, the 7g crash + network-off test, the 7b two-client sync/conflict proof, the §9 Step-1 held-out FSRS calibration (+ SVG reliability chart), and the 7f gold-set card checker with its frozen ship gate — each a one-command `just` recipe writing `eval/` reports |
| `desktop/qt/launcher/src/main.rs` | **Final (shipping)**: the launcher installs wheels bundled at `Contents/Resources/wheels/` when present (this fork's build), instead of `anki-release` from PyPI; stock behaviour otherwise |
| `desktop/qt/aqt/speedrun_study.py` (+ `deckbrowser.py`, `speedrun_dashboard.py`, dashboard `SubjectTable.svelte`) | **Study a CFA topic** (desktop-only): (re)builds an Anki filtered deck from a topic's `cfa::topic::*` tag (+ mapped tags) and enters review — deck gear menu + per-topic dashboard **Study** buttons; reuses the filtered-deck engine, no scheduler change |
| `desktop/tools/speedrun/ladder_notetypes.py` (+ `strip_source_citation.py`) | The ladder card templates no longer display the corpus citation; the **Source** field now holds the `cfa::topic::*` tag. `strip_source_citation.py` migrates an already-imported collection (backs up first) to match |
...
[`desktop/MODEL_DESCRIPTIONS.md`](desktop/MODEL_DESCRIPTIONS.md). A single-page
"what AI we built, why, and what we skipped" summary:
[`desktop/AI_NOTE.md`](desktop/AI_NOTE.md). The runtime assistant layer's
spec, invariants, and per-feature acceptance criteria:
[`desktop/RUNTIME_AI_PLAN.md`](desktop/RUNTIME_AI_PLAN.md).
